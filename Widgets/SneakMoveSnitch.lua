function widget:GetInfo()
   return {
    name      = "SneakMoveSnitch",
    desc      = "Attempt to make Snitch sneak. Version 0,5",
    author    = "terve886",
    date      = "2019",
...
local SnitchUnitDefID = UnitDefNames["shieldbomb"].id
local selectedSneakers = nil

local cmdSneak = {
	id      = CMD_SNEAK,
	type    = CMDTYPE.ICON_MAP,
	tooltip = 'Makes Snitch sneak slowly to target location.',
	cursor  = 'Attack',
	action  = 'reclaim',
	params  = { }, 
...
function widget:UnitCommand(unitID, unitDefID, unitTeam, cmdID, cmdParams, cmdOpts, cmdTag)
	if (cmdID == CMD_SNEAK) then
		for _,Snitch in pairs(SneakerStack) do
			if(Snitch.unitID == unitID)then
...
function widget:UnitFinished(unitID, unitDefID, unitTeam)
		if (UnitDefs[unitDefID].name==Snitch_NAME)
	...
	if selectedSneakers ~= nil then
		if (cmdID == CMD_SNEAK and #params == 3)then
			for i=1, #selectedSneakers do
				for _,Snitch in pairs(SneakerStack) do
					if (selectedSneakers[i] == Snitch.unitID)then
						Snitch:setTargetParams(params)
						Snitch:sneak()
					end
				end
			end
...
function filterSneakers(units)
	...
	for i = 1, #units do
		...
		if (SnitchUnitDefID == GetUnitDefID(unitID)) then
			n = n + 1
			filtered[n] = unitID
		end
	end
